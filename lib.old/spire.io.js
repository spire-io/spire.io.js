...
Spire.prototype.start = function (key, cb) {
  var spire = this;
  this.api.create_session(key, function (err, session) {
...
// @param cb {function(err, spire)} Callback
Spire.prototype.login = function (username, password, cb) {
  var spire = this;
  this.api.login(login, password, function (err, session) {
    if (err) return cb(err);
    spire.session = session;
    cb(null, spire);
  });
...
Spire.prototype.subscribe = function (name, channels, cb) {
  ...
  async.forEach(
    ...
    function (channel, innerCB) {
      spire._findOrCreateChannel(channel, innerCB);
    },
    function (err) {
      if (err) return cb(err);
...
// ## Spire.prototype._findOrCreateSubscription
// Finds or creates a channel
//
// @private
// @param name {string} Subscription name
// @param cb {function (err, channel)} Callback
Spire.prototype._findOrCreateSubscription = function (name, cb) {
