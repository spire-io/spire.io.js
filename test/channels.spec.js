  ...
  describe('Publish to a channel using spire.publish', function () {
    ...
    describe('Listen to a channel using spire.subscribe with no options', function () {
      beforeEach(function () {
        ...
        runs(function () {
          var that = this;
          this.spire.subscribe('another channel', function (messages) {
            that.messages = messages;
            finished = true;
          }, function (err, subscription) {
            that.subscription = subscription;
            setTimeout(function () {
              that.spire.publish('another channel', 'message one', function () {});
            }, 1000);
          });
        });
        ...
        runs(function () {
          ...
          that.spire.publish('yet another channel', 'message uno', function () {
            that.spire.publish('yet another channel', 'message dos', function () {
              that.spire.subscribe('yet another channel', {last: 0, orderBy: 'asc'}, function (messages) {
                that.messages = messages;
                finished = true;
              }, function (err, subscription) {
                that.subscription = subscription;
      ...
      describe('Create a subscription to a channel', function () {
        ...
        describe('Listen for the message we sent', function () {
          beforeEach(function () {
            ...
            runs(function () {
              var that = this;
              this.sub.poll({ last: 0 }, function(err, events) {
                finished = true;
